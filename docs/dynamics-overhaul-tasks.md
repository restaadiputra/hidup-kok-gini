...
- 2026-09-19: Removed the shared payday event. It was always resolved by player 1 as an extra turn right before their real one, so player 1 drew two cards a month. The last payday choice now opens the next month with player 1 rolling. `src/game/payday-flow.test.ts` guards seat order and one card per player per month. Save format is v9.
