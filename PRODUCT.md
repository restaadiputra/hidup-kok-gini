...
- Fonts and artwork are bundled locally. A build-generated service worker precaches every built file, so after one online visit the game reloads and plays offline. It is registered in production builds only, never by the dev server. When a new deploy takes over a tab that is still running an older build, a banner offers **Muat ulang** (`src/update/`). The app checks for a new deploy whenever it returns to the foreground and every 30 minutes; a freshly opened page that already runs the latest build stays quiet. An unknown URL shows a themed 404 page (`public/404.html`) when online.
